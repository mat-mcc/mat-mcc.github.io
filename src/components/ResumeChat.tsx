...
export default function ResumeChat() {
  const [messages, setMessages] = useState<Message[]>([
    { role: 'model', content: "Hello, I'm MattBot! I'm Matthew's AI assistant. Ask me anything about his education, skills, or projects!" }
  ]);
  ...
  return (
    <div className="relative flex flex-col bg-sol-base3 rounded-3xl border border-sol-base1/30 overflow-hidden shadow-2xl h-[600px]">
      ...
      <div className="px-6 py-4 border-b border-sol-base1/20 flex items-center justify-between bg-sol-base2/80 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-mono font-medium text-xs text-sol-base02 flex items-center gap-1">
              matt-bot.ts <span className="text-sol-cyan text-[10px]">v2</span>
            </p>
        ...
        <button
          onClick={() => setMessages([{ role: 'model', content: "Hello, I'm MattBot! I'm Matthew's AI assistant. Ask me anything about his education, skills, or projects!" }])}
          className="p-2 hover:bg-sol-base1/20 rounded-lg transition-colors text-sol-base01 hover:text-sol-base02 cursor-pointer"
          title="Reset chat"
        >
          <RefreshCw size={16} />
        </button>
      ...
      <div ref={scrollRef} className="flex-grow overflow-y-auto p-6 space-y-6 scroll-smooth bg-sol-base3">
        {messages.length === 1 && (
          <div className="space-y-4 pt-2">
            <p className="text-[10px] font-mono text-sol-base01 uppercase tracking-widest">// Some Suggested Queries</p>
            <div className="flex flex-wrap gap-2">
              {[
                { label: "What's his most common tech stack?", color: "border-sol-blue/40 text-sol-blue hover:bg-sol-blue/10" },
                { label: "Tell me about his most recent projects.", color: "border-sol-green/40 text-sol-green hover:bg-sol-green/10" },
                { label: "What are his core strengths?", color: "border-sol-magenta/40 text-sol-magenta hover:bg-sol-magenta/10" },
                { label: "Tell me about his education at Rutgers.", color: "border-sol-yellow/40 text-sol-yellow hover:bg-sol-yellow/10" }
              ].map(q => (
                <button
                  key={q.label}
                  onClick={() => setInput(q.label)}
                  className={cn(
                    "px-3 py-1.5 rounded-xl border text-xs font-mono transition-all bg-sol-base2/50 cursor-pointer",
                    q.color
                  )}
                >
                  "{q.label}"
        ...
        {messages.map((m, i) => (
          ...
          >
            ...
            <div className={`max-w-[80%] p-4 rounded-2xl text-sm leading-relaxed ${m.role === 'user'
              ? 'bg-sol-blue text-white rounded-tr-none shadow-md'
              : 'bg-sol-base2 border border-sol-base1/20 text-sol-base02 rounded-tl-none shadow-sm'
              }`}>
              <div className={cn(
                "markdown-body",
                m.role === 'user'
                  ? "!text-white prose-p:!text-white prose-headings:!text-white prose-strong:!text-white prose-code:!text-sol-yellow prose-code:!bg-black/30 prose-a:!text-sol-cyan"
                  : "text-sol-base02"
              )}>
